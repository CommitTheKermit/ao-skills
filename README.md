...
### run (Codex)
플리파의 로컬 서비스와 Android 에뮬레이터를 함께 준비하고, 대상 앱·이전 테스트·실시간 화면 연결을 확인한다. 정상 실행 중인 구성요소를 재사용한다.

발동 표현: 플리파 문맥에서 "$run", "플리파 실행", "에뮬+서비스 켜줘", "다시 실행".
...
### flow-first (Codex)
사용자 동작을 먼저 합의하고 작은 왕복 흐름 단위로 구현·검증한다. 이슈·디자인·API 사이의 충돌, 인증 후 재개, 저장 후 다른 화면 갱신을 확인하고 최종 명세와 검증 결과를 맞춘다.

발동 표현: `$flow-first`, "플로우 먼저 확인하고 구현", "예상과 다른 플로우 재작업 방지" 등.
원본: [`codex/skills/flow-first/SKILL.md`](codex/skills/flow-first/SKILL.md).
...
> 현재 주차(ISO week)의 변경만 여기 인라인으로 둔다. 지난 주차 이력은 [`changelog/`](changelog/) 의 주차별 파일 참조.

### 2026-09-15 - 신규 추가: `run`
- 기존: 플리파 서비스·에뮬레이터 실행과 세션 복원을 매번 개별 명령으로 처리했음
- 변경: 실행 중인 구성요소 재사용, 대상 AVD 확인, 앱·세션 복원, 인증된 화면 스트림 확인을 Codex `run` 스킬로 정형화
- 이유: 중복 실행과 잘못된 기기 선택을 막고 플리파를 같은 절차로 다시 열기 위해
- 영향 파일: `codex/skills/run/SKILL.md`, `README.md`, `changelog/2026-W37.md`

### 지난 변경내역
- [`2026-W37`](changelog/2026-W37.md) - 2026-09-07 ~ 09-13
