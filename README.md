...
### home-server (Codex)
로컬 SSH 별칭으로 개인 홈서버에 접속해 상태·프로젝트·서비스·예약 작업을 확인하고 요청된 원격 작업을 수행한다. Windows 원격 셸의 최신 `Path`도 확인하며, 접속 정보와 비밀값은 공개 스킬 원본에 저장하지 않는다.
...
- 변경: 공개 원본에는 SSH 별칭만 두고 접속 정보는 로컬 설정으로 분리하며, 원격 상태·Git·서비스·예약 작업 검증과 Windows `Path` 갱신 절차를 표준화
