...
### 2026-06-16 - knowledge-loop 승격 넛지 쿨다운 3일 → 1일
- 기존: `knowledge-nudge.sh` 의 `COOLDOWN=259200`(3일). pending 이 임계값을 넘겨도 한 번 안내 후 3일간 침묵해, 넛지가 안 뜬다고 느껴짐
- 변경: `COOLDOWN=86400`(1일)로 단축. 주석/안내 문구의 "3일" 표기도 "1일"로 통일. `THRESHOLD=8` 은 유지
- 이유: 승격(수동)이 잊혀 후보만 쌓이는 것을 더 자주(하루 한 번까지) 상기시키기 위함
