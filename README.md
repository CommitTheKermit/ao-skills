...
### home-server (Codex)
로컬 SSH 별칭으로 개인 홈서버에 접속해 상태·프로젝트·서비스·예약 작업을 확인하고 요청된 원격 작업을 수행한다. Windows 원격 셸의 `Path`, 권한, 인코딩, 인증 컨텍스트와 TLS 오류를 안전하게 처리하며 접속 정보와 비밀값은 공개 스킬 원본에 저장하지 않는다.

발동 표현: "홈서버 접속", "서버에서 확인", "홈서버에서 실행", "Windows 서버 작업" 등.

### intent-first (Codex)
코드 구현, 수정, 리팩터링, 리뷰에서 코드 길이보다 사용자의 행동과 도메인 의도가 이름, 타입, 책임, 테스트에 명확히 드러나는지를 우선한다. 현재 요구사항을 설명하지 않는 추상화는 추가하지 않는다.

발동 범위: 코드 구현, 수정, 리팩터링, 리뷰 작업 전반.
...
### 2026-09-20 - `home-server` 세션 간 상태 공유
- 기존: 세션마다 서비스와 예약 작업 상태를 개별 명령으로 다시 조회했음
- 변경: 로컬 주소 설정으로 내부 상태 JSON을 읽고 확인 시각과 운영 메모를 공유
- 이유: 실제 최신 상태와 과거 기록을 구별하면서 홈서버 작업을 이어가기 위해
- 영향 파일: `codex/skills/home-server/SKILL.md`, `README.md`

### 2026-09-19 - 신규 추가: `intent-first`
- 기존: 짧은 코드와 최소 구현을 우선하는 규칙은 사용자의 행동과 정책을 코드에 충분히 드러내지 못할 수 있었음
- 변경: 구현과 리뷰에서 사용자 언어, 명시적 결과 타입, 책임 경계, 요구사항으로 읽히는 테스트를 우선하도록 표준화
- 이유: 코드의 분량보다 사용자의 의도와 도메인 규칙이 유지보수 과정에서 선명하게 전달되도록 하기 위해
- 영향 파일: `codex/skills/intent-first/SKILL.md`, `README.md`

### 2026-09-19 - 신규 추가: `home-server`
- 기존: 개인 홈서버 접속 정보와 안전 점검 절차가 프로젝트 문서에 흩어져 원격 작업마다 다시 확인해야 했음
- 변경: 공개 원본에는 SSH 별칭만 두고 접속 정보는 로컬 설정으로 분리하며, Windows SSH 제약과 재부팅 후 서비스 등록 복구를 표준화
