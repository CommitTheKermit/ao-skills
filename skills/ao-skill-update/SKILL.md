...
- **스킬** (`skills/<name>/SKILL.md`): 자동 발동이 핵심 가치이거나 레포 운영에 필수일 때. `description`이 매 대화 상시 로드되어 조건을 만족하면 자동 발동된다. 그만큼 상시 토큰을 쓴다.
- **커맨드** (`commands/<name>.md`): 사용자가 `/이름`으로 명시 호출하는 게 자연스러울 때. description을 짧은 한 줄로 둘 수 있어 상시 토큰이 적다. 자동 발동은 없다.
